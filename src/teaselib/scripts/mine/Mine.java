...
public class Mine extends Player {
    ...
    public Mine(TeaseLib teaseLib, File basePath, String mistressPath) {
        super(teaseLib, new ResourceLoader(basePath, ResourcesRoot, Assets,
    ...
    private void mapToysToMultipleItems() {
        ...
        state.addScriptValueMapping(MappedScriptState.Global,
                new MappedScriptItemValue(389,
                        items(Toys.Cockring).all(Features.Vibrating),
    ...
    private void mapSelfReferencingBodyParts() {
        state.addScriptValueMapping(MappedScriptState.Global,
                new MappedScriptStateValue.Indefinitely(20,
                        state(Body.OnNipples), Body.OnNipples));
        state.addScriptValueMapping(MappedScriptState.Global,
                new MappedScriptStateValue.Indefinitely(21, state(Body.InButt),
                        Body.InButt));
        state.addScriptValueMapping(MappedScriptState.Global,
                new MappedScriptStateValue.Indefinitely(22, state(Body.InMouth),
                        Body.InMouth));
        state.addScriptValueMapping(MappedScriptState.Global,
                new MappedScriptStateValue.Indefinitely(23, state(Body.OnPenis),
                        Body.OnPenis));
        state.addScriptValueMapping(MappedScriptState.Global,
                new MappedScriptStateValue.ForSession(24, state(Body.OnBalls),
                        Body.OnBalls));
        state.addScriptValueMapping(MappedScriptState.Global,
        ...
        state.addScriptValueMapping(MappedScriptState.Global,
                new MappedScriptStateValue.ForSession(27,
                        state(Body.AroundWaist), Body.AroundWaist));
        state.addScriptValueMapping(MappedScriptState.Global,
                new MappedScriptStateValue.ForSession(28,
                        state(Body.AroundNeck), Body.AroundNeck));
        state.addScriptValueMapping(MappedScriptState.Global,
                new MappedScriptStateValue.ForSession(30,
                        state(Body.CantJerkOff), Body.CantJerkOff));
        state.addScriptValueMapping(MappedScriptState.Global,
                new MappedScriptStateValue.ForSession(31,
                        state(Body.CantSitOnChair), Body.CantSitOnChair));
        state.addScriptValueMapping(MappedScriptState.Global,
                new MappedScriptStateValue.ForSession(32, state(Body.CantStand),
                        Body.CantStand));
        state.addScriptValueMapping(MappedScriptState.Global,
                new MappedScriptStateValue.ForSession(33, state(Body.CantKneel),
                        Body.CantKneel));
        state.addScriptValueMapping(MappedScriptState.Global,
                new MappedScriptStateValue.ForSession(34,
                        state(Body.CantTypeOrUseMouse),
                        Body.CantTypeOrUseMouse));
        state.addScriptValueMapping(MappedScriptState.Global,
    ...
    private void clearSessionTimerCheck() {
        PersistentBoolean clearTimeCheck = persistentBoolean(
                Assignments.ClearSessionTimerCheck);
    ...
    private void triggerCarryOutEnema() {
        PersistentBoolean triggerCarryOutEnema = persistentBoolean(
                Assignments.CarryOutEnema);
